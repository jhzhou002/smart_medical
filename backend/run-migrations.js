...
// 基础schema文件（必须先执行）
const baseSchema = 'schema_local.sql';

// 迁移脚本列表（按执行顺序）
const migrations = [
  '001_add_auth_system_local.sql',
  '002_modify_existing_tables_local.sql',
];
...
async function main() {
  ...
  // 1. 先执行基础 schema（创建基础表）
  console.log('📌 步骤 1: 初始化基础数据库表结构\n');
  const schemaPath = path.join(databaseDir, baseSchema);

  try {
    const schemaSql = fs.readFileSync(schemaPath, 'utf8');
    await pool.query(schemaSql);
    console.log(`✅ 基础表结构创建成功: ${baseSchema}\n`);
  } catch (error) {
    console.error(`❌ 基础表结构创建失败: ${baseSchema}`);
    console.error('错误信息:', error.message);
    allSuccess = false;
  }
